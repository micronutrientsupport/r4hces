...
To demostrate the visuals, let us load a dataframe called `ihs5_consumption` which was generated in{@sec-wrangling}.  
...
This function is not part of the `ggplot2` package, therefore it has to be intalled and loaded before using it (For more information about packages see @sec-packages. 
...
First, we are going to plot and save the scatter plot with the faceted region as an object in our enviroment called `graph1`. 
...
plot_grid(graph1, graph2, graph3, nrow = 2)
...
(top_row <- plot_grid(graph2, graph3, ncol = 2, labels = "AUTO"))
...
plot_grid(top_row, graph1, nrow = 2, labels = c("", "C"))

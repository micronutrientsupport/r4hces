...
3. Contatenate the values of `food_name` and `food_subname` and assign the result to a new character object called `full_name`.
...
# Create a numeric object called food_quantity_mg and calculate the value of food_quantity_g in milligrams
...
food_name != food_name3
```
Other logical operators are `>`, `<`, `>=` and `<=`. Logical object can be the subject of logical functions, notably `"if`  .. `then"`.

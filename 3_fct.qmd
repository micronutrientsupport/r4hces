...
### 1. Selecting food composition data

#### 1.1. Selecting the food composition data for use with the survey.
...
##### 1.1.1. Relevancy for the study/context (e.g., geographically and culturally close).
...
##### 1.1.2. FCT availability & missing values (e.g., relevant foods and nutrients are reported)
...
#TODO: Can we do this interactive? :::{.callout-note} \## Question 2 How many foods are reported in the Tanzania FCT?
...
##### 1.1.3. Data quality and reporting (e.g., method of analysis, good metadata)
...
Please note: in the code chunck below, the 'template' is a fill-in value for a folder - if you wish to store this file in a folder, you must create a folder in your here::here location (found by running "here::here()") and replace 'template' with that folders name. It is also important to make sure that the filename you pick for the downloaded file has the same file suffix (e.g. '.xlsx') as the file you are downloading.
...
## 4.1. Deriving one variable from another (e.g. Food Group from Food Item ID)
...
### 4.1.1. Extracting food group names
